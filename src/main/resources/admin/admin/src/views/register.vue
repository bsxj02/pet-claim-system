	...
	<div>
		<div class="container" :style='{"minHeight":"100vh","padding":"20px","alignItems":"center","background":"url(http://codegen.caihongy.cn/20240129/17e5d014970b46b0a6da2629973d8845.png) no-repeat","display":"flex","width":"auto","backgroundSize":"cover","justifyContent":"flex-end"}'>
			<el-form v-if="pageFlag=='register'" :style='{"boxShadow":"0 1px 20px rgba( 255,  255, 255, .8)","padding":"20px 0","margin":"0px 140px 0px 0px","borderRadius":"4px","background":"#fff","width":"400px","height":"auto"}' ref="rgsForm" class="rgs-form" :model="rgsForm" :rules="rules">
				<div v-if="true" :style='{"width":"100%","margin":"0 0 10px 0","lineHeight":"44px","fontSize":"20px","color":"#374254","textAlign":"center"}' class="title">基于JavaWeb的宠物救助及领养平台的设计与实现注册</div>
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='jiuzhuzhe'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('jiuzhuzhanghao')?'required':''">救助账号：</div>
					<el-input  v-model="ruleForm.jiuzhuzhanghao"  autocomplete="off" placeholder="救助账号"  type="text"  />
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='jiuzhuzhe'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('mima')?'required':''">密码：</div>
					<el-input  v-model="ruleForm.mima"  autocomplete="off" placeholder="密码"  type="password"  />
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='jiuzhuzhe'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('jiuzhurenming')?'required':''">救助人名：</div>
					<el-input  v-model="ruleForm.jiuzhurenming"  autocomplete="off" placeholder="救助人名"  type="text"  />
				</el-form-item>
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='jiuzhuzhe'">
					<div v-if="false" :style='{"width":"64px","lineHeight":"44px","fontSize":"14px","position":"relative","color":"rgba(64, 158, 255, 1)"}' class="lable" :class="changeRules('xingbie')?'required':''">性别：</div>
                    <el-select v-model="ruleForm.xingbie" placeholder="请选择性别" >
                        <el-option
                            v-for="(item,index) in jiuzhuzhexingbieOptions"
                            v-bind:key="index"
                            :label="item"
                            :value="item">
                        </el-option>
				...
				<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' class="list-item" v-if="tableName=='jiuzhuzhe'">
                    ...
                    <file-upload
                        ...
                        :multiple="true"
                        :fileUrls="ruleForm.touxiang?ruleForm.touxiang:''"
                        @change="jiuzhuzhetouxiangUploadChange"
                    ></file-upload>
				</el-form-item>
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if ('jiuzhuzhe' == this.tableName) {
				this.rules.jiuzhuzhanghao = [{ required: true, message: '请输入救助账号', trigger: 'blur' }]
			}
			if ('jiuzhuzhe' == this.tableName) {
				this.rules.mima = [{ required: true, message: '请输入密码', trigger: 'blur' }]
			}
			if ('jiuzhuzhe' == this.tableName) {
				this.rules.jiuzhurenming = [{ required: true, message: '请输入救助人名', trigger: 'blur' }]
			}
			this.yonghuxingbieOptions = "男,女".split(',')
			this.jiuzhuzhexingbieOptions = "男,女".split(',')
		}
	},
	created() {
	},
	...
	methods: {
        ...
        jiuzhuzhetouxiangUploadChange(fileUrls) {
            this.ruleForm.touxiang = fileUrls;
        },
		...
		login() {
            ...
            if(this.ruleForm.touxiang!=null) {
                this.ruleForm.touxiang = this.ruleForm.touxiang.replace(new RegExp(this.$base.url,"g"),"");
					...
					if((!this.ruleForm.jiuzhuzhanghao) && `jiuzhuzhe` == this.tableName){
						this.$message.error(`救助账号不能为空`);
						return
					}
					if((!this.ruleForm.mima) && `jiuzhuzhe` == this.tableName){
						this.$message.error(`密码不能为空`);
						return
					}
					if((this.ruleForm.mima!=this.ruleForm.mima2) && `jiuzhuzhe` == this.tableName){
						this.$message.error(`两次密码输入不一致`);
						return
					}
					if((!this.ruleForm.jiuzhurenming) && `jiuzhuzhe` == this.tableName){
						this.$message.error(`救助人名不能为空`);
						return
					}
					if(`jiuzhuzhe` == this.tableName && this.ruleForm.lianxidianhua &&(!this.$validate.isMobile(this.ruleForm.lianxidianhua))){
