...
<div class="content" :style='{"padding":"30px"}'>
	...
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","flexWrap":"wrap","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":" 1px solid rgba(167, 180, 201,.3)    ","margin":"0 10px","borderRadius":"4px","background":"#fff","display":"flex"}' v-if="isAuth('liulangdongwu','首页总数')">
			...
			<div :style='{"width":"120px","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex","order":"1"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"1","fontWeight":"bold","height":"24px"}'>{{liulangdongwuCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"14px","color":" #a3b1c9","height":"24px"}'>流浪动物总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="liulangdongwuChart1" class="echarts1" v-if="isAuth('liulangdongwu','首页统计')"></div>
		<div id="liulangdongwuChart2" class="echarts2" v-if="isAuth('liulangdongwu','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getliulangdongwuCount();
		this.liulangdongwuChat1();
		this.liulangdongwuChat2();
	},
	methods:{
		// 词云
		...
		getliulangdongwuCount() {
			this.$http({
				url: `liulangdongwu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.liulangdongwuCount = data.data
				}
			})
		},
		liulangdongwuChat1() {
        ...
        var liulangdongwuChart1 = echarts.init(document.getElementById("liulangdongwuChart1"),'macarons');
        this.$http({
            url: "liulangdongwu/group/zhuangtai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                liulangdongwuChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    liulangdongwuChart1.resize();
                };
            }
        });
      })
    ...
    liulangdongwuChat2() {
      this.$nextTick(()=>{
        ...
        var liulangdongwuChart2 = echarts.init(document.getElementById("liulangdongwuChart2"),'macarons');
        this.$http({
            url: "liulangdongwu/group/chongwuzhonglei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].chongwuzhonglei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].chongwuzhonglei
                    })
                }
                var option = {};
                ...
                // 使用刚指定的配置项和数据显示图表。
                liulangdongwuChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    liulangdongwuChart2.resize();
                };
            }
        });
      })
